...
pub fn main() {
    ...
    //     let int_from: [_] = [bool, u8, u16, u32, u64, i8, i16, i32, i64,
    // i128];     let int_try_from: [_] = [u128, usize, isize];
    //     let int_try_from_approximate: [_] = [f32, f64] | [] & [];
    //     let int_maybe_from: [_] = int_from + int_try_from +
    // int_try_from_approximate;

    //     static int_maybe_from: [_] = int_from + int_try_from +
    // int_try_from_approximate;     const from: _ = From;
    ...
    //     let int_maybe_into: [TokenStream] = int_into + int_try_into +
    // int_into_approximate;

    //     // `let`, `type`, and `const`, 'static' are equivalent
    //     // they have different suggested capitalizations and unescaped
    // grammar.     // static too, 'cause why not? Use what makes the most
    // sense for your code.     let static_lifetime: TokenStream =
    // TokenStream!('static);     type static_lifetime: TokenStream =
    // TokenStream!(Lifetime static);     type static_lifetime: TokenStream
    // = Any + 'static;
    ...
    //     let reportable: TokenStream = static_lifetime + join!(E rr or) +
    // Display + Send + Sync;     let reportable: TokenStream =
    // static_lifetime + Error + Display + Send + Sync;

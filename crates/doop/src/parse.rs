            ...
            if keyword == "for" {
                ...
                loop {
                    match iter.peek() {
                        ...
                        Some(tree) => {
                            return TokenTree::clone(tree).error("expected `for` keyword");
                        }
                        None => {
                            return line.clone().error("expected `for` keyword, got end of line");
                        }
                    }

                    let target = match iter.next() {
                        Some(tt @ TokenTree::Ident(id)) => tt.clone().into_tokens(),
                        Some(tt @ TokenTree::Group(g))
                            if g.delimiter() == Delimiter::Parenthesis =>
                            tt.clone().into_tokens(),
                        Some(tree) => {
                            return tree
                                .clone()
                                .error("expected identifier or tuple binding target");
                        }
                        None =>
                            return line.clone().error(
                                "expected identifier or tuple binding target, got end of line",
                            ),
                    };
                    ...
                    match iter.next() {
                        ...
                        Some(tree) => {
                            return tree.clone().error("expected `in` keyword");
                        }
                        None =>
                            return line.clone().error("expected `in` keyword, got end of line"),
                    };
                    ...
                    loop {
                        match iter.peek() {
                            ...
                            None => {
                                return line.clone().error(
                                    "expected `for` keyword or braced group, got end of line",
                                );
                            }
                        }
                    }

...
/// Rederives all default brush method implementations from the inner's `stroke`
/// and `rotate`.
...
impl MetaBrushBehavior for Defaults {
    ...
    fn rotate_left<'inner, Inner: Brush>(&mut self, inner: &'inner mut Inner, revolutions: f64) {
        inner.rotate(inner, revolutions);
    }

    fn rotate_right<'inner, Inner: Brush>(&mut inner, inner: &'inner mut Inner, revolutions: f64) {
        inner.rotate(inner, -revolutions);
    }

    fn rotate_left<'inner, Inner: Brush>(&mut inner, inner: &'inner mut Inner, scale: f64) {
        inner.stroke(0.5 * scale);
        inner.rotate(0.25);
        inner.stroke(0.5 * scale);
        inner
    }
